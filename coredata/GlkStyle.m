...
- (GlkStyle *)clone {
    ...
    cloned.attributeDict = [self.attributeDict mutableCopy];
    cloned.autogenerated = YES;
	return cloned;
}
...
- (NSSize)cellSize {
    ...
    NSFont *font = [self.attributeDict objectForKey:NSFontAttributeName];
    NSSize size;

    // This is the only way I have found to get the correct width at all sizes
    if (NSAppKitVersionNumber < NSAppKitVersionNumber10_8) {
        size = [@"X" sizeWithAttributes:self.attributeDict];
    } else
        size = [font advancementForGlyph:(NSGlyph)'X'];

    NSLayoutManager *layoutManager = [[NSLayoutManager alloc] init];
    size.height = [layoutManager defaultLineHeightForFont:font] + ((NSParagraphStyle *)[self.attributeDict objectForKey:NSParagraphStyleAttributeName]).lineSpacing;
    return size;
}
...
-(void)createDefaultAttributeDictionary {
    ...
    if ([self testGridStyle]) {
        NSLog(@"This is a grid style. Setting font to Monaco");
        font = [NSFont fontWithName:@"Monaco" size:12];

    } else {
        NSLog(@"This is a buffer style. Setting font to Helvetica");
    ...
    [dict setObject:@(style_Normal) forKey:@"GlkStyle"];
            ...
            switch (i) {
                ...
                case stylehint_ReverseColor:
                    ...
                    if ([[hints objectAtIndex:stylehint_TextColor] isEqual:[NSNull null]] &&
                        ...
                        NSColor *bgcolor = [self.attributeDict objectForKey:NSBackgroundColorAttributeName];
                        NSColor *fgcolor = [self.attributeDict objectForKey:NSForegroundColorAttributeName];
...
- (void)printDebugInfo {
    NSInteger stylevalue = ((NSNumber *)[self.attributeDict objectForKey:@"GlkStyle"]).integerValue;
    NSString *styleName = [self testGridStyle] ? [gGridStyleNames objectAtIndex:stylevalue] : [gBufferStyleNames objectAtIndex:stylevalue];
    NSColor *debugColor = [self.attributeDict objectForKey:NSForegroundColorAttributeName];
    NSString *fontColorName = [debugColor isEqual:[NSColor blackColor]]?@"black":@"not black";
    if ([debugColor isEqual:[NSColor whiteColor]]) {
        fontColorName = @"white";
    }
    debugColor = [self.attributeDict objectForKey:NSBackgroundColorAttributeName];
    NSString *backgroundColorName = [debugColor isEqual:[NSColor blackColor]]?@"black":@"not black";
    if ([debugColor isEqual:[NSColor whiteColor]]) {
        fontColorName = @"white";
    }

    NSFont *debugFont = [self.attributeDict objectForKey:NSFontAttributeName];
    NSLog(@"GlkStyle printDebugInfo: theme: %@ style: %ld (%@). Font: %@ Font size: %f Foreground color:%@ Background color: %@", [self findTheme].themeName, stylevalue, styleName, debugFont.fontName, debugFont.pointSize, fontColorName, backgroundColorName);
}
