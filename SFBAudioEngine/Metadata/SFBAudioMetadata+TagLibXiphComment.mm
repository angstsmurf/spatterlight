...
namespace {
	...
	TagLib::ByteVector DecodeBase64(const TagLib::ByteVector& input)
	{
		...
		if(!decoder) {
			fprintf(stderr, "SecDecodeTransformCreate failed: %{public}@", error.Object());
			return {};
		}
		...
		if(!SecTransformSetAttribute(decoder, kSecTransformInputAttributeName, sourceData, &error)) {
			fprintf(stderr, "SecTransformSetAttribute failed: %{public}@", error.Object());
			return {};
		}
	...
	TagLib::ByteVector EncodeBase64(const TagLib::ByteVector& input)
	{
		...
		if(!encoder) {
			fprintf(stderr, "SecEncodeTransformCreate failed: %{public}@", error.Object());
			return {};
		}
		...
		if(!encodedData) {
			fprintf(stderr, "SecTransformExecute failed: %{public}@", error.Object());
			return {};
		}
