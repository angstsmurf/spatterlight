...
/* Private map document extension (SVG automap); see automap-api.md */
#define gestalt_Map (0x1104)

#define mapimage_SVG (1)
...
/* Private map-document events (gestalt_Map). win == NULL;
...
/* Map document extension (gestalt_Map). SVG/PNG/JPEG via glk_map_present;
   ...
   image size. glk_map_fill_rect draws a solid RGB color rect (same overlay
   ...
   (0xFFFFFFFF) for library default (SVG fill / window background). */
...
extern glui32 glk_map_present(glui32 format, const unsigned char *data, glui32 len,
