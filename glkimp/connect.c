...
static char *css_pack_sel_prop_val(const char *sel, glui32 sellen,
                                   ...
                                   const char *val, glui32 vallen, size_t *outlen)
{
    ...
    if (!val)
        vallen = 0;
    len = (size_t)sellen + 1 + (size_t)proplen + 1 + (size_t)vallen + 1;
    buf = malloc(len);
    if (!buf)
        return NULL;
    if (sellen)
        memcpy(buf, sel, sellen);
    buf[sellen] = '\0';
    memcpy(buf + sellen + 1, prop, proplen);
    buf[sellen + 1 + proplen] = '\0';
    if (vallen)
        memcpy(buf + sellen + 1 + proplen + 1, val, vallen);
    buf[sellen + 1 + proplen + 1 + vallen] = '\0';
    *outlen = len;
    return buf;
}
                  ...
                  const char *val, glui32 vallen)
{
    ...
    if (!buf)
        return;
    win_flush();
    sendmsg(CSSHINT, wintype, styl, span_or_par, 0, 0, len, buf);
    free(buf);
}
                        ...
                        const char *prop, glui32 proplen)
{
    ...
    if (!buf)
        return;
    win_flush();
    sendmsg(CLEARCSSHINT, wintype, styl, span_or_par, 0, 0, len, buf);
    free(buf);
}
...
{
    size_t len = 0;
    char *buf = css_pack_sel_prop_val(sel, sellen, prop, proplen, val, vallen, &len);

    if (!buf)
        return;
    win_flush();
    sendmsg(CSSSELECTORHINT, wintype, 0, 0, 0, 0, len, buf);
    free(buf);
}
...
{
    size_t len = 0;
    char *buf = css_pack_sel_prop_val(sel, sellen, prop, proplen, NULL, 0, &len);

    if (!buf)
        return;
    win_flush();
    sendmsg(CLEARCSSSELECTORHINT, wintype, 0, 0, 0, 0, len, buf);
    free(buf);
}
                                        ...
                                        glui32 sellen)
{
    ...
    buf[sellen] = '\0';
    win_flush();
    sendmsg(CLEARALLCSSHINTBYSELECTOR, wintype, 0, 0, 0, 0, len, buf);
    free(buf);
}
...
{
    win_flush();
    sendmsg(CLEARALLCSSHINTBYWINDOW, wintype, 0, 0, 0, 0, 0, NULL);
}

void win_css_hint_clear_all_inline(int name)
{
    win_flush();
    sendmsg(CLEARALLCSSINLINE, name, 0, 0, 0, 0, 0, NULL);
}
                        ...
                        glui32 proplen, const char *val, glui32 vallen)
{
    ...
    if (!buf)
        return;
    win_flush();
    sendmsg(SETCSSINLINE, name, span_or_par, 0, 0, 0, len, buf);
    free(buf);
}
                          ...
                          glui32 proplen)
{
    ...
    if (!buf)
        return;
    win_flush();
    sendmsg(CLEARCSSINLINE, name, span_or_par, 0, 0, 0, len, buf);
    free(buf);
}
