/* Glk mapping extension (gestalt_Map). SVG/image present + overlays. */
...
/* Wire blob for MAPPRESENT (SVG only):
     u32 bgcolor          (mapcolor_Default = theme buffer background)
     u32 data_len
     u8  data[data_len]   (UTF-8 SVG)
     u32 nhyperlinks
     repeated:
       u32 id
       u32 npoints          (>= 3)
   ...
   MAPOVERLAYSVG buffer (a1=id a2=left a3=top a4=width a5=height):
     u32 zindex
     u32 link_id
     u32 linklabel_len
     u8  linklabel[linklabel_len] (UTF-8)
     u32 svg_len
     u8  svg[svg_len]     (UTF-8 SVG)
 */
    ...
    const glk_maphyperlink_t *hyperlinks, glui32 nhyperlinks)
{
    ...
    win_flush();
    sendmsg_glk(MAPPRESENT, (int)focusleft, (int)focustop,
                (int)focuswidth, (int)focusheight, 0, off, (char *)pack);
    return 1;
}

glui32 glk_map_present_image(glui32 image, glui32 bgcolor,
    ...
    const glk_maphyperlink_t *hyperlinks, glui32 nhyperlinks)
{
    ...
    win_flush();
    sendmsg_glk(MAPPRESENTIMAGE, (int)focusleft, (int)focustop,
                (int)focuswidth, (int)focusheight, 0, off, (char *)pack);
    return 1;
}
...
overlayid_t glk_map_overlay_svg(const unsigned char *data, glui32 len,
                                glsi32 left, glsi32 top, glui32 width, glui32 height,
                                glui32 zindex, glui32 link_id, const char *linklabel)
{
    static unsigned char pack[GLKBUFSIZE];
    size_t off = 0;
    glui32 i;
    ...
    win_flush();
    /* a1=id a2=left a3=top a4=width a5=height; pack=zindex,link_id,label,svg */
    sendmsg_glk(MAPOVERLAYSVG, (int)id, (int)left, (int)top, (int)width, (int)height,
                off, (char *)pack);
    return id;
}
...
{
    return win_map_get_visibility();
}

void glk_map_show_at_user_request(void)
{
    win_flush();
    sendmsg_glk(MAPSHOWATUSERREQUEST, 0, 0, 0, 0, 0, 0, NULL);
}
