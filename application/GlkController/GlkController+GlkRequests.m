...
#pragma mark Glk mapping extension
        ...
        case MAPPRESENT: {
            ...
            if (req->a3 > 0 && req->a4 > 0) {
                focus = [MapFocusRect new];
                focus.left = req->a1;
                focus.top = req->a2;
                focus.width = (NSUInteger)req->a3;
                focus.height = (NSUInteger)req->a4;
            }

            [map presentSVG:svg bgcolor:(NSUInteger)bgcolor
        ...
        case MAPPRESENTIMAGE: {
            ...
            [map presentImage:img bgcolor:(NSUInteger)bgcolor
                        focus:focus hyperlinks:hyperlinks];
            break;
        }
        ...
        case MAPOVERLAYSVG: {
            ...
            size_t blen = (size_t)req->len;
            size_t off = 0;
            glui32 zindex = 0, linkId = 0, labellen = 0, svglen = 0;
            NSString *label = nil;
            NSImage *img = nil;
            BOOL ok = blen >= 16;
            if (ok) {
                ...
                if (off + labellen + 4 > blen)
                    ...
                    if (off + svglen > blen)
                        ...
                        NSData *svgData = [NSData dataWithBytes:bytes + off length:svglen];
                        img = svgData ? [[NSImage alloc] initWithData:svgData] : nil;
                        if (!img)
                            ok = NO;
                    } else {
                        ok = NO;
            ...
            if (!ok) {
                NSLog(@"glkctl: MAPOVERLAYSVG: corrupt or unreadable SVG overlay");
                break;
            }
            MapOverlay *ov = [MapOverlay new];
            ov.overlayId = (NSUInteger)req->a1;
            ...
            ov.left = req->a2;
            ov.top = req->a3;
            ov.width = (NSUInteger)MAX(req->a4, 0);
            ov.height = (NSUInteger)MAX(req->a5, 0);
            ov.zindex = (NSUInteger)zindex;
            ov.linkId = (NSUInteger)linkId;
            ov.label = label;
            [map setOverlay:ov];
        ...
        case MAPGETVISIBILITY:
            ans->cmd = OKAY;
            ans->a1 = (self.mapWindowController && self.mapWindowController.mapVisible) ? 1 : 0;
            break;

        case MAPSHOWATUSERREQUEST:
            [[self ensureMapWindowController] showMapAtUserRequest];
            break;
