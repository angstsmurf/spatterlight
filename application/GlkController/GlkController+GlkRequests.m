...
#pragma mark Map document extension
        ...
        case MAPPRESENT: {
            ...
            size_t off = 0;
            BOOL ok = YES;
            glui32 format = mapimage_SVG;
            glui32 bgcolor = mapcolor_Default;

            if (off + 4 > blen)
                ok = NO;
            else {
                format = (glui32)bytes[off]
            ...
            if (ok && dataLen > 0) {
                if (format == mapimage_SVG) {
                    ...
                    if (!svg)
                        svg = @"";
                } else if (format == mapimage_PNG || format == mapimage_JPEG) {
                    imageData = [NSData dataWithBytes:bytes + off length:dataLen];
                } else {
                    ok = NO;
                }
                off += dataLen;
            ...
            if (format == mapimage_SVG) {
                ...
                NSImage *img = imageData ? [[NSImage alloc] initWithData:imageData] : nil;
                if (!img) {
                    NSLog(@"glkctl: MAPPRESENT: could not decode PNG/JPEG payload");
                    break;
                }
