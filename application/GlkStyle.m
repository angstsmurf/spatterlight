...
- (void)encodeWithCoder:(NSCoder *)encoder {
    ...
    NSMutableArray *enablearray =
        [NSMutableArray arrayWithCapacity:stylehint_NUMHINTS];
...
- (void)prefsDidChange {
    ...
    if (windowtype == wintype_TextGrid) {
        dict =
            [[Preferences attributesForGridStyle:(int)stylenumber] mutableCopy];
    } else {
        dict = [[Preferences attributesForBufferStyle:(int)stylenumber]
            mutableCopy];
    }
    ...
    if (enabled[stylehint_ReverseColor] &&
        !(enabled[stylehint_TextColor] || enabled[stylehint_BackColor])) {
        if (windowtype == wintype_TextGrid) {
            [dict setObject:[Preferences gridBackground]
                     forKey:NSForegroundColorAttributeName];
            [dict setObject:[Preferences gridForeground]
                     forKey:NSBackgroundColorAttributeName];
        } else {
            [dict setObject:[Preferences bufferBackground]
                     forKey:NSForegroundColorAttributeName];
            [dict setObject:[Preferences bufferForeground]
                     forKey:NSBackgroundColorAttributeName];
        }
    }
}
