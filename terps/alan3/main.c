...
static void readTemporaryHeader(ACodeHeader *tmphdr) {
    if (fseek(codfil, 0, SEEK_SET) != 0)
        playererr("Could not rewind Alan game file");
    if (fread(tmphdr, sizeof(*tmphdr), 1, codfil) != 1)
        playererr("Could not read Alan game file header");
    if (fseek(codfil, 0, SEEK_SET) != 0)
        playererr("Could not rewind Alan game file");
...
char *decodedGameVersion(char version[]) {
    static char str[100];
    snprintf(str, sizeof(str), "%d.%d%s%d",
             (int)version[3],
             (int)version[2],
...
static void incompatibleDevelopmentVersion(ACodeHeader *header) {
    char str[500];
    snprintf(str, sizeof(str), "Incompatible version of ACODE program. Development versions always require exact match. Game is %ld.%ld%s%ld, interpreter %ld.%ld%s%ld!",
...
static void nonDevelopmentRunningDevelopmentStateGame(char version[]) {
    ...
    strncpy(errorMessage, "Games generated by a development state compiler", sizeof(errorMessage) - 1);
    errorMessage[sizeof(errorMessage) - 1] = '\0';
    snprintf(versionString, sizeof(versionString), "(this game is v%d.%d.%d%s)", version[0], version[1],
            version[2], decodeState(version[3]));
    strncat(errorMessage, versionString, sizeof(errorMessage) - strlen(errorMessage) - 1);
    strncat(errorMessage, "can only be run with a matching interpreter. Look for a game file generated with an alpha, beta or release state compiler.>\n", sizeof(errorMessage) - strlen(errorMessage) - 1);
    apperr(errorMessage);
}
