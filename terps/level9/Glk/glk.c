...
gln_gameid_identify_game (void)
{
   ...
   * The alternative conditions are unique for Colossal Adventure v2 for MSX.
   */
  assert (startdata);
  ...
  if (gln_graphics_enabled)
    {
      ...
      if (!gln_graphics_active)
        {
          if (gli_slowdraw && gln_graphics_interpreter_state == GLN_GRAPHICS_LINE_MODE) {
            GLN_GRAPHICS_TIMEOUT = 1;
          } else {
...
#ifdef GARGLK
  garglk_set_program_name("Level 9 5.2");
  garglk_set_program_info(
      "Level 9 5.2 by Glen Summers, David Kinder\n"
