...
public:
	...
	const byte *dataPtr() const {
		// Use data()+_pos rather than &_data[_pos] so a position at the very
		// end of the buffer yields a valid past-the-end pointer instead of
		// tripping libc++'s hardened operator[] bounds check.
