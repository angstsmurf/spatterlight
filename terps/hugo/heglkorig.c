...
int hugo_displaypicture(HUGO_FILE infile, long reslength)
{
	...
	if (hugo_hasgraphics())
	{
		...
		if (buf)
		{
			...
			int x0 = ggridmarginx + picleft * gcellw;
			int y0 = ggridmarginy + pictop * gcellh;
			int x1 = ggridmarginx + picright * gcellw;
			int y1 = ggridmarginy + picbottom * gcellh;
			
			if (width > x1 - x0) { width = x1 - x0; height = width * aspect; }
			if (height > y1 - y0) { height = y1 - y0; width = height / aspect; }
			
			int xoff = ((x1 - x0) - width) / 2;
			int yoff = ((x1 - x0) - width) / 2;
						
			fprintf(stderr, "hugo_displaypicture w=%d h=%d\n", width, height);
			fprintf(stderr, "  inwin=%d [ %d %d %d %d ]\n", inwindow, 
					picleft,
					pictop,
					picright,
					picbottom);

			/* in a window ... create layered picture window (but ... mainwin is not sized correctly) */
			if (0 && inwindow)
			{
				picwins[npicwins] = win_newwin(wintype_Graphics);
				if (picwins[npicwins] >= 0)
				{
					win_sizewin(picwins[npicwins], x0, y0, x1, y1);
					win_drawimage(picwins[npicwins], xoff, yoff, width, height);
					npicwins ++;
				}
			}
			
			/* no window set ... fallback to main window */
			else
			{
				win_drawimage(mainwin->peer, imagealign_InlineUp, 0, width, height);
			}
		}
	}
...
void hugo_settextwindow(int left, int top, int right, int bottom)
{
	...
	if (picright > SCREENWIDTH - 0x0f)
		picright = (gscreenw - ggridmarginx * 2) / gcellw;
	if (picbottom > SCREENHEIGHT - 0x0f)
		picbottom = (gscreenw - ggridmarginx * 2) / gcellw;
