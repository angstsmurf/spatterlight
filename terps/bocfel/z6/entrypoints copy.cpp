...
void find_entrypoints(void) {
    ...
    fprintf(stderr, "find_entrypoints: address 0x9de8 holds value 0x%x\n", memory[0x9de8]);
            ...
            if (entrypoint.pattern.size()) {
                ...
                if (offset != -1) {
                    ...
                    if (entrypoint.stub_original) {
                        ...
                        fprintf(stderr, "Overwriting byte at offset 0x%04x (0x%x) with 0xb0\n", entrypoint.found_at_address, memory[entrypoint.found_at_address]);
    ...
    fprintf(stderr, "after find_entrypoints: address 0x9de8 holds value 0x%x\n", memory[0x9de8]);
}
