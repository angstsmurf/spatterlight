...
// Reads one null-terminated chunk out of the file (the original used
// QuetzalReader::readString, which does the same thing).
static String readPascalString(Common::ReadStream *f) {
	String result;
	while (!f->eos()) {
		byte c = f->readByte();
		if (c == 0)
...
bool progfile::readChar(char &ch) {
	if (last_ch != NULL_CH) {
		...
		while (line_pos >= (int)line_buffer.size()) {
			...
			line_buffer = readPascalString(&_file);
