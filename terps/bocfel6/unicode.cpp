...
static unsigned unicode_entries = 69;
...
void parse_unicode_table(uint16_t utable)
{
    ...
    for (unsigned i = 0; i < unicode_entries; i++) {
...
static void build_unicode_to_zscii_tables()
{
    ...
    for (unsigned i = 0; i < unicode_entries; i++) {
        ...
        if (valid_unicode(c)) {
            unicode_to_zscii  [c] = (unsigned char)i + 155;
            unicode_to_zscii_q[c] = (unsigned char)i + 155;
        }
    }

    // Now the values that are equivalent in ZSCII and Unicode.
    for (unsigned i = 32; i < 127; i++) {
        unicode_to_zscii  [i] = (unsigned char)i;
        unicode_to_zscii_q[i] = (unsigned char)i;
    }
...
static void build_unicode_to_latin1_table()
{
    ...
    for (unsigned i = 0; i < 256; i++) {
        unicode_to_latin1[i] = (unsigned char)i;
    }
}
...
static void build_alphabet_table()
{
    ...
    for (unsigned i = 0; i < 52; i++) {
        atable_pos[atable[i]] = (int)i;
    }
    for (unsigned i = 53; i < 26 * 3; i++) {
        atable_pos[atable[i]] = (int)i;
    }
}
