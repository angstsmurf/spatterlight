...
A2File *firstfile = NULL;
A2File *lastfile = NULL;
...
uint8_t *ReadInfocomV6File(uint8_t *data, size_t *len, int *game, int *diskindex)
{
    ...
    int i;
    for (i = 0; title[i]; i++) {
        size_t length = strlen (title[i]);
        if (!strncmp (fVolumeName, title[i], length)) {
    ...
    char filename[64];
    snprintf(filename, 64, "%s.D%d", title[i], index);
    ...
    if (file) {
        ...
        size_t actual;
        ProDOSRead(file, buf, file->fDirEntry.blocksUsed * kBlkSize - 1, &actual);
        *len = actual;
    }
