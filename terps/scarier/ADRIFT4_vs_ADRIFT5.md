...
All markup is stripped to plain text by `a5text_render_plain()`. There are **no `os_print_tag` calls from the v5 path** — the Glk host receives plain UTF-8 via `gsc_a5_put_string()`. In interactive mode, presentation tags survive as positional sentinel marks (a5text.h): `<cls>`, `<waitkey>`, `<center>`/`<right>`/`<b>`/`<i>` spans, `<c>`/`<font color>` colour spans (`A5_COLOR_MARK`), `<window>` routing, and `<wait N>` timed pauses (`A5_WAIT_MARK`, run through the same cancelable Glk timer delay as the v4 `<wait x.x>` tag — Death Shack types its title one 80-point letter per `<wait 3>`). Adventure-level OLE colours become Glk stylehints before the window opens; inline colours use `garglk_set_zcolors`.
