...
- `harness/scsertest.c`: every new field round-trips through
  save->restore (`ScarierBattleState/2`); `/1` saves still read.
...
values already there. Bump the marker to `ScarierBattleState/2` and keep reading
...
- `scarier-battle-system-port` memory — `.tas` save layout RE and decompile
