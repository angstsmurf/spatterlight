...
SCARIER historically did **not** implement ADRIFT's optional Battle System — the
...
project SCARIER derives from, has no combat code at all.
...
(`adrift-battle-formulas-re`, `scarier-battle-system-port`).
...
4. **Player death flow.** DONE. `battle_kill` for the player now prints
   ...
   `has_completed = TRUE`, ending the game through SCARIER's normal completion
...
5. **Save/restore of live stamina.** DONE for SCARIER's own save/restore. The
   ...
   ("ScarierBattleState/1"). `ser_load_game` first re-rolls via `battle_start`,
   then, if the sentinel follows the turns count, overrides with the saved
   values. The sentinel's absence — a real ADRIFT save, or a SCARIER save from
   ...
   **Still open (full Runner interop):** matching this byte-for-byte is blocked
   ...
   1. SCARIER's stream already omits ADRIFT's leading version line
      (`<chr 172>` + `major`/`minor`/`rev`), so SCARIER `.tas` files are not
      Runner-loadable, and SCARIER cannot load Runner saves, irrespective of
      battle data.
   2. The interleaved blocks are mostly the **mutable** Strength/Defence/
      ...
      action edits at runtime; SCARIER does not implement that action and re-rolls
   ...
   "none" on load (`global_78 = &HFF`); SCARIER's choice to persist it is a benign
...
8. **Mutable battle attributes (the "Change `<attribute>`" task action).**
   ...
   Strength / etc. at runtime; SCARIER currently parses-and-discards it
...
7. **RNG fidelity note (won't-fix-able).** Combat is RNG-driven and the Runner
   uses VB `Rnd()`; SCARIER uses `sc_randomint`, so turn-by-turn results can't be
...
the console interpreter with `clang -O2 -w sc*.c os_ansi.c -lz -o scarier` (the
bundled `zlib123.zip` is absent; link the system `-lz`). Battle game corpus
lives at `~/adrift-battle/games/` (see `scarier-battle-system-port` memo).
