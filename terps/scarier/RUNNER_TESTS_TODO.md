...
| Single-object library success vs failing explicit-verb task | library acts (e.g. `take rock` takes the rock when no task passes) | prints the task's fail message: `take rock` with a failing `take * rock` task answers TFAIL and the rock stays put; `wear rock` likewise WFAIL | **NEW divergence, observed live 2026-08-02 (probe `FM4`), documented not fixed.** run400 lets a matched-but-failing task claim single-object take/wear even though the library action could complete — the mirror image of the put-family rule above — and it does NOT rewrite `take` to `get` (a passing `get * rock` task did not fire on `take rock` there, where Scarier's canonical-verb retry fires it).  Zero corpus impact (every row passes); reconciling it would mean reworking the take/drop retry model run400's own put behavior contradicts, so it stays documented until a game demands it. |
| Zero-length always-restarting event tick timing | end of turn, after the command — sees the state the command just made | before the command — a `time1=0,time2=0` checker event reacts one turn later | **Observed 2026-08-02** in `TheADRIFTProject`: its `#Pill Check` event runs a checker task each turn; on the author-route `put pill in cup` turn run400 prints only the library put (the radioactive-mix chain fires next turn via `take slime`'s own action chain), while Scarier's end-of-turn tick fires the mix on the put turn itself.  Outcome-identical (same +10, same 90/100, same win) — golden blessed with the one-turn-earlier flavor text; not fixed. |
