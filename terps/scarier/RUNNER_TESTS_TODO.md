...
- [x] **Scarier deliberately does NOT implement the Runner's completed-`*`
      ...
      **Superseded 2026-09-13 — PORTED.** The refusal above is withdrawn:
      `run_spent_task_390()` (`scrunner.cpp`) now makes the claim where
      run390's `checktask` (44B6E8) makes it — before movement and the
      standard library, for any pattern (a bare `*` included), printing the
      task's RepeatText or the default openadv installs at load
      (465A8B..465AB9).  The decompile also settled *how* the claim works:
      the done arm (44B4DD) writes the RepeatText into the answer buffer
      (44B537) and jumps to 44B6CC, which is *inside* the loop above `Next`
      (44B6DA), so the scan continues — a later live task that passes its
      restrictions still runs (44B663; that is how Journ2's T5 hands over the
      King of Hearts after T3 is spent), and a later failing restriction's
      non-empty FailMessage overwrites the buffer (452BB8, 44B636; that is
      why inverness's later `knock`s print their own refusals).  inverness now
      soft-locks exactly as run390 does (`Adrift_1030_inverness.txt`): the
      route ends on the cutscene's last line, 75 banked but unprintable.
      journ2 5/90, vampire 70/100, merry_murders 120/135, circus /
      cybercow_win / fugitive re-blessed, mr_smith re-routed; nothing else
      in the suite moved.
...
| Completed non-repeatable `*` task | skipped by matcher and event dispatch | claims every later command: "You have already done that." — soft-locks inverness for real | ~~**Deliberate divergence.** Do not import; see §2.~~ **PORTED 2026-09-13** as `run_spent_task_390()` (run390 `checktask` 44B4DD..44B537, ahead of movement and the library): the done arm writes the RepeatText — or the load-time default — into the answer buffer and the scan *continues*, so a later live task that passes still runs and a later failing restriction's message overwrites it. Goldens re-anchored: journ2 5/90, vampire 70/100, merry_murders 120/135, inverness walls on the cutscene line; see §2's superseding note. |
...
**2026-09-13:** below 4.0 the already-done half no longer runs last.
`run_spent_task_390()` makes the claim in `run_all_commands()` *before* the
movement and standard-library handlers — where run390's `checktask` makes it —
for any command pattern (wildcards included) and with the task's RepeatText or
the load-time default; `run_task_refusal()`'s done arm is now a pre-4.0
fallback and keeps the room refusal and the 4.0 RepeatText path.  The
literal-only / default-only / movement-exempt narrowing of 2026-08-10 is gone,
and with it the four walkthroughs it kept winning (see §2).
