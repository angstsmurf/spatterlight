...
| `*` matching **zero** words in a task command | matches: `put * pill in cup` claims the input `put pill in cup` | appears not to — the same input falls through to the library | **NEW, found 2026-08-02 while deriving `TheADRIFTProject`. Not yet probed live.** The game's TASK 41 `#Put Pill IN Slime` has `ALTCMD[3]=[put * pill in cup]`; none of its other alternatives can match `put pill in cup` (ALTCMD[1]'s final set is `[slime/goo/gooze/ooze]`, and `cup` is not in it). Scarier matches it with `*` = ∅, the restrictions fail (they want the slime in the cup already), and the turn is spent printing the fail message "Not yet." The author's own comp transcript shows the plain library reply "You put the small pill inside the coffee cup." on that exact input, i.e. the Runner did not match the task at all. Two readings fit: (a) the Runner's `*` requires ≥1 word, or (b) it matches but falls through to the library when the *failing* restriction has no message of its own — note the connector string is `#A#A#O#`, so the failing branch is not the whole expression. Corpus impact so far: one game, one turn, worth 20 points (`TheADRIFTProject_solution.txt` reorders two commands around it and documents why). Probe candidate: author a one-task .taf with `verb * noun` and feed it `verb noun` in run400. |
