...
| Pre-3.9 loss and death messages | prints `Better luck next time.` / `I'm afraid you are dead!` in every version | `Better luck next time.` is genuinely **absent from run370 and run380** — it first appears in run390. The death line is a different story: run370/380/390 all carry the *pair* `I'm afraid ` + ` dead!` and assemble the sentence around a perspective word, and only run400 holds `I'm afraid you are dead!` as one literal | **Open, narrowed 2026-08-15, still zero measured exposure.** The first census for this row searched for `afraid you are dead` and so missed the assembled form; an exact-literal census of all four binaries (`I'm afraid `, ` dead!`, `Better luck next time.`, `You are dead!`, `Congratulations!`) is what the claim above now rests on. So the death branch is probably fine in every version *for a second-person game* and diverges only for perspective 1 or 3, where the Runner would say "I'm afraid I am dead!" / "…he is dead!" and Scarier says "you are"; the **loss** branch is the one that is likely printing a line no pre-3.9 Runner has. Still needs a 3.7/3.8 EndGame probe (`probes37/mkprobe37c.py`, `make38probe.py`) — no corpus game reaches a pre-3.9 loss or death, which is why this has stayed cheap to leave open. Corroboration that 3.9 *does* print the loss line: `druggy_lane.taf` (3.90) carries an ALR `Better luck next time.` → `Hope you do better next time.` |
| Where a 3.9 EndGame task's WinText goes | on its own line under the task's CompleteText | **pre-4.0** joins them into **one** flowed paragraph — run390 `END scwin.  You have won.`, and run380 does the same on `microwaveman.taf` — where run400 puts WinText on its own line, which is what Scarier does | **Open, cosmetic, spotted 2026-08-15** while measuring the summary above (`make_39_endprobe.py` under run390), and widened the same day from 3.9 to the whole pre-4.0 range by the run380 replay. Same shape as the other pre-4.0 sentence-joining rows; not chased. It is *visible* in the corpus (any pre-4.0 game with both a CompleteText and a WinText on the winning task) but every such golden was blessed from Scarier, so nothing fails today. |
...
| A trailing `AdditionalMessage` the Runner never prints | prints it | prints nothing — for **one** task in one game | **Open, unexplained, no ported change, measured live 2026-08-15.** `jb2000.taf` (3.80) task 14 `LAND IT` wins the game and carries the author's sign-off `" THANK YOU!  THAT WAS MY GAME!  BY CHRIS STEPHENSON ..."` as its AdditionalMessage (field 5 of the task's text block, line 1103 of the deobfuscated file). run380 finishing the game prints the task's CompleteText and goes straight to the score summary — the sign-off never appears. Four cells rule out the obvious causes: (a) run380 *does* print AdditionalMessage on an ordinary task — `microwaveman.taf` task 0 `hold up glass`, where it is flowed onto the end of the CompleteText paragraph; (b) run380 *does* print it on a task that ends the game — `marooned.taf` task 29, above; (c) it is not the text — a patched `jbt.taf` whose AdditionalMessage is the single word pair `TRAILER TEST` is equally silent; (d) it is not the ending — a second patch clearing the task's `WinGame` flag leaves the game running and the trailer *still* does not print. So this is specific to that task's record, and the likeliest explanation is that our 3.80 schema mislocates the field for it rather than that the Runner has a rule we have not found. Zero further corpus exposure: jb2000 is the only game where the two disagree. |
| A task's `ShowRoomDesc` prints the room **name** | prints the room name as a heading, then the description | prints the **description only** | **Open, one cell, spotted live 2026-08-15 while measuring the row above.** `topaz.taf` task 22 `wear ring` has `ShowRoomDesc` pointing at room 7, Forest Clearing. run400 prints `Sun shines down through the leaves, ...` directly under the task's CompleteText, with no `Forest Clearing` heading and no blank line between the two; the blessed golden has both. Not ported on a single cell — `lib_print_room_name()` is shared with ordinary movement, where the heading is certainly right, so this needs a second game (or a probe) before the ShowRoomDesc path is split off from it. Corpus exposure is unmeasured — `SCR_DUMP_TASKS` does not yet print `ShowRoomDesc`, and adding it is the first step if this is picked up. |
| Trailing blank lines of a text block before the summary | collapses them — a CompleteText ending `<br><br>` yields one blank line before the summary | keeps them and adds its own line terminator, so the same text yields **two** | **Open, cosmetic, spotted 2026-08-15.** Measured by pixel rows on the run390 `ECOD3.taf` ending (36px line pitch; `In an alley behind Denny's.` to `You scored 0…` is 108px, i.e. three pitches). `castle.taf` and `microwaveman.taf`, whose ending texts have no trailing `<br>`, show the summary on the very next line in the Runner and in Scarier alike, so the divergence is only in how many trailing newlines survive. Likely the same root as the row above — worth re-measuring once the end-of-turn ordering is ported, because the Runner's message is appended to a turn's accumulated text, not to a task's. |
...
(Var1 + 1, the Runner's own biased byte) and prints it from
`task_print_end_game_message()` at the foot of the main loop.  chicago's live
