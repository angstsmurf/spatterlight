...
| Where an event's LookText sits **inside** the room block | inside the description paragraph, before the object list and the character lines: `A bare arena.  K1 LOOK.` / `Also here is a rock.` / `Robot is here…` | **dead last, after everything**: `A bare arena.  Also here is a rock.  Robot is here, looking dangerous.  K1 LOOK.` | **NEW divergence, probed live 2026-08-02, deliberately unfixed.**  Found while settling the row above; probes `EV7` (adds a floor object to `EV6`) and `EV8` (adds an NPC as well) in run400, with run390 agreeing on the 3.9 twin (`LONG.  Also here is a rock and a slime.  K1 LOOK.`).  Exposure is the 65 LookText-bearing events in 19 corpus games, but only in rooms that also list objects or characters.  Left alone on purpose: the LookText loop lives inside `lib_cmd_look()`'s description block (`sclibrar.cpp`), and moving it past `lib_print_room_contents()` is a *spacing* change as much as an ordering one — the Runner runs it on into the character lines with the usual two-space separator, which the contents printer does not currently leave the buffer in a position to do.  Worth doing on its own, with its own re-bless, not folded into the start-model fix. |
...
probes that proved it also turned up a *second*, smaller divergence, now its
own documented-not-fixed row: both Runners print event LookText **last** in
the room block, after the object list and the character lines, where Scarier
prints it inside the description paragraph.
