...
| When the end-of-game message is printed | inline, at the moment the EndGame **action** runs, before the task's remaining actions | at the **end of the turn**: `Form1.checkx` calls `Form1.evaluate` for the whole turn and only then, if the gameover byte is set, `Form1.endmessage` (`0x0005C681`) | **Open, measured live 2026-08-15, real corpus exposure.** `chicago.taf` task 23 is `ACT type=6` (EndGame, win) followed by `ACT type=4 v1=10` (+10 points). run390 ends the game reporting **75**; Scarier reports **65**, because it prints the summary at the action and then stops. A scan of the corpus for tasks whose EndGame action is not the last one finds **42 tasks in 32 games**, most of them the same `[6, 4]` shape, so this moves numbers in more goldens than chicago's. Porting it means holding the pending ending (and its Var1) in game state and emitting it after the turn's tasks and events have run, instead of from `task_run_end_game_action()`. |
