...
| When immediate events start relative to the opening room description | they start during the first tick, i.e. **after** the opening description: their StartText prints below it, and their LookText is missing from it | run400 starts them during load, **before** the description: the opening room text carries their LookText, their StartText is nowhere to be seen (printed into the pre-intro screen and cleared), and only the finish half lands under the description.  Probe `EV5` turn 0, run400: `A bare arena.  H1 LOOK.  H2 LOOK.  H1 FINISH.  H1 TASK.  H1 START.  …` against Scarier's `A bare arena.` / `H1 START.` / `H1 FINISH.` / `H1 TASK.` / … | **Open, unprobed beyond the zero-length probes** — noticed 2026-08-02 while settling the row above, and deliberately left alone: it is not a zero-length question at all.  Any event with a LookText and an immediate start would show it, so the blast radius is the whole corpus's turn-0 output, and the fix (start immediate events at load, discarding their StartText) needs its own probe with a *non*-zero-length immediate event before anyone touches the tick order.  Corpus exposure is **unmeasured** — every golden here is derived from Scarier itself, so a clean run proves nothing about this; the number that matters is how many games give an immediately-started event a LookText, and nobody has counted. |
