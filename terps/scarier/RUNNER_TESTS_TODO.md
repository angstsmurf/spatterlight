...
| Pre-3.9 loss and death messages | prints `Better luck next time.` / `I'm afraid you are dead!` in every version | **run370 and run380 have neither string**; `Better luck next time.` first appears in run390, and the death line is assembled from `I'm afraid ` + ` dead!` (perspective-aware) in run390, only run400 holding it as one literal | **Open, zero measured exposure so far.** Found by the four-binary census done for the summary row above. It is not safe to conclude 3.7/3.8 print *nothing* — the ending may use a differently-worded literal elsewhere in those binaries. Needs a 3.7/3.8 EndGame probe (`probes37/mkprobe37c.py`, `make38probe.py`); the 3.9 side is already covered by `make_39_endprobe.py`. |
| Where a 3.9 EndGame task's WinText goes | on its own line under the task's CompleteText | run390 joins them into **one** line, two spaces between: `END scwin.  You have won.` (run400 puts WinText on its own line, which is what Scarier does) | **Open, cosmetic, spotted 2026-08-15** while measuring the summary above (`make_39_endprobe.py` under run390). Same shape as the other 3.9 sentence-joining rows; not chased, and no corpus row is known to turn on it. |
