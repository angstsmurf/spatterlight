...
| The Euripides Enigma | 427 | diverge | 11 (xo 11) | **WIRED + FIXED 2026-07-02** — full 400-point win (`*** CONGRATULATIONS! ***`, 426 turns) from the game's **own built-in walkthrough** (`WLKTHRGH`, `cl_Walkthroug1`) with two script corrections: dropped a spurious `hit fork on face` (it *closes* the already-open canyon opening in this build; the `4`-conversation desync the wiring TODO flagged was just a downstream symptom of being stuck there) and `put pistol in pistol holster`→`holster pistol` (ambiguous phrasing). **Fixed an exponential ALR-recursion hang**: ~25 identity ALRs (a 25-asterisk banner, `OldText`==`NewText`) branched `process_inner` ~25-fold/level to the depth-8 cap (>60 s CPU); ported FD's `If sText = sALR Then Exit For` guard (bail before recursing when the whole text equals the replacement). Residual 11 (both RNG modes ⇒ real bugs): a dropped object already named in room prose ("boom box"/"small drone") is also appended to the "Also here is …" auto-list (+ drone containment), one doubled cliff-face description, one squeeze-through wording variant. See TODO_a5_walkthrough_bugs.md (top entry). |
