...
Run from terps/scarier/test/adrift4.  Deterministic under SCR_SEED=2.
"""
...
base = open("goldens/light_up_solution.txt").read().splitlines()
...
for pads5 in range(0, 10):
    ...
    if "THE END" in out:
        ...
        open("goldens/light_up_solution_new.txt", "w") \
            .write("\n".join(cmds) + "\n")
        print("wrote goldens/light_up_solution_new.txt")
        sys.exit(0)
