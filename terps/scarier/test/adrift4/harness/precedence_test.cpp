 ...
 *
 * Regression test for scr_does_command_match(), the probe that lets the Glk
 ...
 * when it would expand an abbreviation -- we check that scr_does_command_match()
...
probe_west_room (void)
{
  ...
  expect ("west room: game claims 'p' (don't expand to open)",
          scr_does_command_match (g_game, "p"), 1);
  expect ("west room: game does NOT claim 'k'",
          scr_does_command_match (g_game, "k"), 0);
  expect ("west room: game does NOT claim 'c'",
          scr_does_command_match (g_game, "c"), 0);
  expect ("west room: game does NOT claim 'x'",
          scr_does_command_match (g_game, "x"), 0);
}
...
probe_east_room (void)
{
  ...
  expect ("east room: game claims 'k' (don't expand to attack)",
          scr_does_command_match (g_game, "k"), 1);
  expect ("east room: game does NOT claim 'p'",
          scr_does_command_match (g_game, "p"), 0);
}
