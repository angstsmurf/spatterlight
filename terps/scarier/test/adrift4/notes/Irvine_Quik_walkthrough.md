# Irvine Quik & the Search for the Fish of Traglea — walkthrough (**WIN, no score, 172 commands**)
...
- **Harness row:** `iqsfot_solution.txt|iqsfot.taf|Thus one courageous space
  cadet saved the fish|SCR_SEED=31 SCR_SKIP_WAITKEY=1`, PASSing golden.
...
* `breathe` also takes one off per turn, but it is refused unless Irvine is
  ...
  moment. Going straight through is cheaper. The 2026-09-13 route peaks at
  damage 8 (HP 4) and leaves the palace at damage 7.
...
172 commands. `goldens/iqsfot_solution.txt`.
...
| 115–153 | route: `punch sentinel`, `n`, `n`, `e`, `s`, `s`, `s`, `e`, `x crates`, `deploy`, `get key`, `get key`, `deploy`, `w`, `n`, `n`, `n`, `n`, `w`, `n`, `w`, `unlock door`, `w` — with 16 KOs woven in | the palace, BACKDOOR → CORRIDOR → the black tube down to the STORAGE ROOM for the important-looking key (the same HiRBy two-step as the glasses), back north, west to the THRONE HALL ENTRANCE, unlock, `w` → **chapter 6**. The weave depends on the seed's respawn phasing and is generated by `harness/iqsfot_palace.py`: any mook sharing the room gets its correct attack (sentry `punch`, guard `kick`, patrol `throw`, soldier `punch`), otherwise the next route step. The elite is walked past |
| 154–157 | `teach fan karate`, `give jacket to fan`, `ask for help`, `w` | the fan servant. Optional by the PDF's own admission; kept because the epilogue calls back to the given-away jacket |
| 158–165 | `x fat cat`, `x top hat`, `x end table`, `x mirror`, `deploy`, `get mirror`, `look`, `get mirror` | ROYAL THRONE ROOM. The fat cat monologues with a microwave cannon pointed at you; the examines are the stall, and the mirror (again, HiRBy fetches, Irvine takes) turns the cannon back on him |
| 166–169 | `n`, `x reservoir`, `get drowsy fish`, `get tranquil fish` | the SECRET AQUARIUM behind the bookshelf — the fish of Traglea, at last |
| 170–172 | `s`, `e`, `o` | out past the wreckage of the throne room to the PALACE GATES → EPILOGUE → **THE END** |

## Reproducing

```sh
cd terps/scarier/test/adrift4
