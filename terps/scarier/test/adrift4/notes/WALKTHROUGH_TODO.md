...
goldens re-blessed plus `test/adrift4/harness/capacity_nest_expected.txt`; two of the rewrites
...
is ADRIFT 5 and already covered by `test/adrift5/harness/run_a5_walkthroughs.sh`; the other
...
`goldens/to_hell_and_beyond_assisted_solution.txt` was never broken — the
...
- **Stale-claim correction:** `goldens/to_hell_and_beyond_assisted_solution.txt`
...
unaffected by the binary refresh) and `~/scare/test/adrift4/harness`.
...
- Banked `goldens/through_time_solution.txt` (deterministic tour to the wall) +
...
(`goldens/circus_solution.txt`, `SC_SEED=2`; verified via `play.sh`; marker *"PETA
...
- **Economy is the unfinished part**: need 9 tokens (4 duck + 1 funhouse + 4 reading),
  ...
  Bill absent and nothing moves Bill. Banked `goldens/circus_solution.txt` (seed 2) reaches
...
Deterministic winning walkthrough banked in `goldens/shadowpeak_solution.txt`; full
...
`Shadowpeak_walkthrough.md`; solution `goldens/shadowpeak_solution.txt`. Began the
...
`Space_Boy_walkthrough.md`; solution `goldens/space_boy_solution.txt` (145 cmds).
...
`goldens/les_feux_solution.txt`). **Cracked all mechanics, banked a verified
...
**Confirmed opening spine (deterministic, banked in `goldens/circus_solution.txt`,
...
All runs via `safeplay.sh`. Tree: only docs + `goldens/circus_solution.txt` +
...
(`terps/scare/test/adrift4/`) as the single source of truth; the
...
`goldens/screen_savers_solution.txt` (132 cmds). Native **ADRIFT 3.9** TechTV fan
...
`Matts_House_walkthrough.md`; solution `goldens/matts_house_solution.txt` (7
...
> `goldens/alexis_solution.txt` is no longer the 101-cmd 23/65 script described
> below; it is the 171-cmd carry-the-cube route.

`ALEXIS_walkthrough.md`; solution `goldens/alexis_solution.txt` (101 cmds).
...
`SRSintro_walkthrough.md`; tour solution `goldens/srsintro_solution.txt`.
...
`tcom_walkthrough.md`; solution `goldens/tcom_solution.txt` (13 cmds). ADRIFT
...
`Bomb_Threat_walkthrough.md`; solution `goldens/bomb_threat_solution.txt`. An
...
`Theannihilationofthink2_walkthrough.md`; solution `goldens/think2_solution.txt`.
...
`Melbourne_Beach_walkthrough.md`; solution `goldens/melbourne_beach_solution.txt`.
...
`WesGHN_walkthrough.md`; solution `goldens/wes_ghn_solution.txt`. *Wes Garden's
...
`Light_Up_walkthrough.md`; solution `goldens/light_up_solution.txt`. *Light Up:
...
`FunHouse_walkthrough.md`; solution `goldens/funhouse_solution.txt`. A tiny
...
`thetest_walkthrough.md`; solution `goldens/thetest_solution.txt`. A whimsical
...
`Main_Course_walkthrough.md`; solution `goldens/maincourse_solution.txt`. By
...
+ `goldens/space_boy_solution.txt` (deterministic; re-verify with `play.sh`).
...
- **Villains & Kings** is **31/37 unassisted** (was "13/37 faithful, 30/37
  ...
  `goldens/villains_and_kings_assisted_solution{,.expected}.txt` are retired;
...
- [x] **The_X-Files_A_New_Beginning** — **WON, full game verified end-to-end at
      ...
      `goldens/xfiles_solution.txt`. Linear conversation story, no combat. Act 1
...
- [x] **lifesimulation** (35 KB) — **0/0 sandbox, no win** (like Azra/Nonsense).
      ...
      `goldens/lifesimulation_solution.txt`. Open-ended life-sim (apartment +
...
- [x] **The_Spirits_Flight** (37 KB) — **UNWINNABLE; max reachable 50/95**
      ...
      `goldens/spirits_flight_solution.txt`. Battle System (faithful, non-zero
...
- [x] **inverness** (45 KB) — *Inverness Castle* v0.3c, a Macbeth Act 1–2
      ...
      `goldens/inverness_solution.txt`. **Zero EndGame (type-6) actions** ⇒ no
...
- [x] **SecretOfLostWorld** (49 KB) — **WON, full 3300/3300**, deterministic.
      ...
      `goldens/secret_of_lost_world_solution.txt` (1st line = gender answer
...
- [x] **Toxically_Earth** (51 KB) — **WIN reached, deterministic; 0/0 (no
      ...
      `goldens/toxically_earth_solution.txt` (11 cmds). Tobias Schmitt's surreal
...
- [x] **gateway** (81 KB) — *Gateway: Guardian Child* by Michael R. Grice. **WON,
      ...
      `goldens/gateway_solution.txt` (1st two lines = name `Hero` + gender
...
- [x] **Phoenix_Destiny** (121 KB) — *Phoenix Destiny: Book One* by Chris Tyson
      ...
      solution `goldens/phoenix_destiny_solution.txt` (13 leading blank lines
...
- [x] **hyper_b_s** (132 KB) — **WON, full 100/100, deterministic.** Walkthrough
      `hyper_b_s_walkthrough.md`; solution `goldens/hyper_b_s_solution.txt`. Not a
...
- [x] **Shadow_Of_The_Past** (272 KB — largest file, but only embedded graphics;
      ...
      `goldens/shadow_of_the_past_solution.txt`. A reborn-hero cave-escape: free
...
- [x] **Sun_Empire_Quest_For_The_Founders** — 140/145, win verified. Walkthrough
      `Sun_Empire_walkthrough.md`; solution `goldens/sun_empire_solution.txt`.
...
- [x] **Orient_Express** ("The Oriental Express") — 300/300, win verified.
      ...
      `goldens/orient_express_solution.txt`. (Comedy jewel-heist on a train;
...
- [x] **The_Nonsense_Machine_6000** — *not a game.* Toy random-nonsense
      ...
      solution `goldens/the_nonsense_machine_6000_solution.txt`.
- [x] **The_Search_For_Mr_Smith** — **WIN 90/100** (re-derived 2026-08-02;
      ...
      `goldens/mr_smith_solution.txt`; marker = the victory line
...
- [x] **The_Town_Of_Azra** — unfinished RPG sandbox; **no score (0/0), no win**.
      ...
      `goldens/the_town_of_azra_solution.txt`. Of the author's 6 intro goals,
...
- [x] **To_Hell_And_Beyond** — **UNWINNABLE** (max reachable ~68/373).
      ...
      `goldens/to_hell_and_beyond_solution.txt`. Large (190 rooms) but its whole
...
- [x] **Villains_And_Kings** — **31/37**, no win ending (re-derived 2026-08-02;
      ...
      `goldens/villains_and_kings_solution.txt`. **Cause of the old error: this
