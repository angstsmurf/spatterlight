...
`~/scare/test/adrift4/harness`; the 36 other adrift-battle solutions were
...
the gargoyles. Route appended to `goldens/shadowpeak_solution.txt` (re-verified
deterministic, 245):
...
drawbridge to the castle entrance. All steps are in `goldens/shadowpeak_solution.txt`
...
`goldens/shadowpeak_solution.txt` (command-only). Beats, in order:
...
> Run: `sh harness/play.sh games/"Shadowpeak.taf" goldens/shadowpeak_solution.txt`
>
...
**Route (all in `goldens/shadowpeak_solution.txt`).** From room 137 (rainbow):
...
canonical `goldens/shadowpeak_solution.txt` was **not touched** (re-verified 505/790).
...
route was found. Canonical `goldens/shadowpeak_solution.txt` untouched (505/790).
