...
#
# This is the tool that converged test/adrift5/goldens/SpectreOfCastleCoris_walkthrough.txt to
...
#
# Usage:  python3 test/adrift5/harness/spectre_prayer_solver.py <command-list.txt> [budget-secs]
#   (run from terps/scarier; needs test/adrift5/harness/a5run_dump and the game blorb; writes
#    the converged list to solved_<status>.txt next to the input.)
#
import subprocess, sys, re, os, time

SCARIER = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
GAME = f"{SCARIER}/test/adrift5/games/TheSpectreOfCastleCoris.blorb"
BIN = f"{SCARIER}/test/adrift5/harness/a5run_dump"
...
def room_names():
    try:
        ...
        dump = subprocess.run([f"{SCARIER}/test/adrift5/harness/a5dump", GAME], capture_output=True, text=True, timeout=60)
