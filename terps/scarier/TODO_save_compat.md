...
Today neither direction works: SCARIER reads and writes its own `.tas` dialect
which is the ADRIFT v4 layout **minus the leading version line** (and with the
Battle System state in a different place), so a Runner save and a SCARIER save are
...
Pure interop nicety: carry a save between Spatterlight/SCARIER and the Windows
...
- **§2 + §3 Runner-format serializer — DONE, validated against real saves.**
  ...
  **§4 CONFIRMED:** a SCARIER-written save (score 33, turns 7) loads cleanly in the
  ...
  now validated **both** directions (Runner→SCARIER byte-exact; SCARIER→Runner
...
the literal SCARIER now writes. (Other v4 builds differ only in the revision
...
acceptance* this is cosmetic — VB `Val()` ignores the spaces — so SCARIER need only
...
1. **The "off-by-one" is the player NAME** (line 8) — `saveg` prints
   ...
   field, which SCARIER omits entirely. So the player block is name + 8 ints, not
...
2. **Static objects serialise as a room LIST, not a single position.** The
   ...
   room it is present in (the `var_1B8(28)` per-room bool array). SCARIER writes a
   single position int for *every* object, so its object stream diverges from the
   Runner's for static objects. SCARIER already models this (`obj_is_static`,
...
current totals (not SCARIER's `0`) is the faithful choice.

## What is already in place

- **Compression — done, both directions.** ADRIFT v4 `.tas` files are
  zlib/deflate streams. SCARIER already deflates on save (`scserial.c` `ser_flush`,
...
- **Mutable battle attributes — done.** The interleaved battle blocks are mostly
  ...
  Speed values; SCARIER now models all of these as live state (`sc_battle_t`), so
  there is finally somewhere to round-trip them. (Previously SCARIER re-rolled them
...
(2 digits), e.g. `¬40006`. SCARIER omits this line entirely — `ser_load_game`
...
- **Write:** emit the header line first, before GameName. The version digits
  ...
  Runner-produced save (and from the v4.0 `.taf` header SCARIER already parses in
...
- **Read:** before reading GameName, read the first line; if it begins with
  ...
  continue. If it does **not**, it is a legacy SCARIER save — rewind one line and
  proceed as today. This presence/absence of the `¬` byte is a clean, unambiguous
  discriminator, so old SCARIER saves keep loading for free (see §5).

### 2. Battle System block placement — interleaved vs trailing

SCARIER writes its battle state as a sentinel-introduced **trailing** block
(`ScarierBattleState/2`) after the turn count. The Runner **interleaves** it,
...
instead of (or in addition to) the trailing block. Note SCARIER's internal
...
(`global_78 = &HFF`). SCARIER persisting it is a benign superset; in Runner-format
output it must be dropped (or kept only as a SCARIER-private extension, see §5).
...
- **Encumbrance.** SCARIER writes four constants `90, 0, 90, 0` and ignores them on
...
- **Object openness/state.** SCARIER writes openness/state only when non-zero
...
- **Player block field count — RESOLVED (it is the player NAME).** The Runner's
  ...
  score — `var_19C(4) (0) (16) (12) (30) (32) (36) (34) (38)` — vs SCARIER's
...
- whether the loader *uses* the four encumbrance fields or recomputes them
  (decides if SCARIER's `90/0/90/0` constants are safe — §3);
- what the ninth player field is (§3);
- whether trailing lines after the turn count are tolerated or cause a hard error
  (decides whether the SCARIER-private `ScarierBattleState` block can coexist with a
  Runner-readable body — §5).

For the *write* direction (SCARIER→Runner) the `saveg` writer is already a complete
and authoritative spec, so SCARIER→Runner does not strictly need 472CA4; it is the
Runner→SCARIER robustness questions that do.

### 5. Migration / dual-format strategy (recommended)

Make the **canonical** SCARIER save format the Runner-compatible one, and keep a
backward-compatible reader:

- **Save:** always write the Runner layout (version header + interleaved battle
  blocks + verified body). Optionally still append the `ScarierBattleState/2`
  trailing block *iff* §4 shows the Runner tolerates trailing data — that lets a
  SCARIER save carry the few SCARIER-only extras (e.g. wielded weapon) while staying
  Runner-loadable; otherwise drop it.
- **Restore:** sniff line 1. `¬…` ⇒ Runner/new format (parse header + interleaved
  blocks). No `¬` ⇒ legacy SCARIER save ⇒ current code path (GameName first,
  trailing `ScarierBattleState/1|2` block). One reader, both formats, no flag-day
...
- **Locale/number formatting**: VB `Format(x, "General Number")` vs SCARIER's
...
- **Round-trip (SCARIER→SCARIER):** unchanged — existing saves and freshly written
  ones both restore (the `scsertest`-style harness in `~/adrift-battle/harness/`).
- **SCARIER→Runner:** write a save from SCARIER for a corpus game, load it in the
...
- **Runner→SCARIER:** the reverse — save in the Runner, restore in SCARIER, diff the
  resulting game state.
- **Legacy SCARIER saves** still load (the §5 sniff).
- **Corpus smoke:** no crashes across the 17 battle games + a sample of
  ...
  SCARIER-written save against a Runner-written one for the same starting state.
